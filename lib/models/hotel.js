    ...
    try {
      let res = await knex.table(`${TBL3}`)
        ...
        `))
          ...
          //   distinct CASE WHEN testing.corridor.corridor_id IS NOT NULL THEN 
          //     jsonb_build_object(
          //         'id',testing.corridor.corridor_id,
          //         'floor_id',testing.corridor.floor_id,
          //         'hotel_id',testing.corridor.hotel_id,
          //         'is_main_corridor',testing.corridor.is_main_corridor

          //       )
          //     END
          // )::TEXT, '[null]'), '[]')::JSON corridor
            resolve(res);
    } catch (err) {
      console.log("err", err);
